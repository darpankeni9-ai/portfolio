(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const l of a)if(l.type==="childList")for(const n of l.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&s(n)}).observe(document,{childList:!0,subtree:!0});function e(a){const l={};return a.integrity&&(l.integrity=a.integrity),a.referrerPolicy&&(l.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?l.credentials="include":a.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function s(a){if(a.ep)return;a.ep=!0;const l=e(a);fetch(a.href,l)}})();const m=document.querySelector("#app"),u=()=>{const i=document.getElementById("particles-js");if(!i)return;const t=document.createElement("canvas");t.style.width="100%",t.style.height="100%",i.appendChild(t);const e=t.getContext("2d");let s=[];const a=()=>{t.width=window.innerWidth,t.height=window.innerHeight};a(),window.addEventListener("resize",a);class l{constructor(){this.x=Math.random()*t.width,this.y=Math.random()*t.height,this.vx=(Math.random()-.5)*.5,this.vy=(Math.random()-.5)*.5,this.radius=Math.random()*2+1,this.opacity=Math.random()*.5+.2,this.color=["#8B5CF6","#06B6D4","#EC4899","#3B82F6"][Math.floor(Math.random()*4)]}update(){this.x+=this.vx,this.y+=this.vy,(this.x<0||this.x>t.width)&&(this.vx*=-1),(this.y<0||this.y>t.height)&&(this.vy*=-1)}draw(){e.beginPath(),e.arc(this.x,this.y,this.radius,0,Math.PI*2),e.fillStyle=this.color,e.globalAlpha=this.opacity,e.fill(),e.globalAlpha=1}}const n=()=>{s=[];const o=Math.floor(t.width*t.height/15e3);for(let r=0;r<o;r++)s.push(new l)},c=()=>{for(let o=0;o<s.length;o++)for(let r=o+1;r<s.length;r++){const v=s[o].x-s[r].x,p=s[o].y-s[r].y,x=Math.sqrt(v*v+p*p);x<150&&(e.beginPath(),e.moveTo(s[o].x,s[o].y),e.lineTo(s[r].x,s[r].y),e.strokeStyle=`rgba(139, 92, 246, ${.1*(1-x/150)})`,e.lineWidth=.5,e.stroke())}},d=()=>{e.clearRect(0,0,t.width,t.height),s.forEach(o=>{o.update(),o.draw()}),c(),requestAnimationFrame(d)};n(),d()},f=()=>{const i=document.querySelector(".cursor-glow");if(!i)return;let t=0,e=0,s=0,a=0;document.addEventListener("mousemove",n=>{t=n.clientX,e=n.clientY});const l=()=>{s+=(t-s)*.1,a+=(e-a)*.1,i.style.left=s+"px",i.style.top=a+"px",requestAnimationFrame(l)};l()},h=()=>{const i=document.querySelector(".typing-text");if(!i)return;const t=["Data Analyst","Power BI Expert","SQL Specialist","Python Developer"];let e=0,s=0,a=!1,l="";const n=()=>{const c=t[e];a?(l=c.substring(0,s-1),s--):(l=c.substring(0,s+1),s++),i.textContent=l;let d=a?30:80;!a&&s===c.length?(d=2e3,a=!0):a&&s===0&&(a=!1,e=(e+1)%t.length,d=500),setTimeout(n,d)};n()},g=()=>{const i={root:null,rootMargin:"0px",threshold:.1},t=new IntersectionObserver(e=>{e.forEach(s=>{s.isIntersecting&&(s.target.classList.add("visible"),s.target.querySelector(".progress-fill")&&s.target.querySelectorAll(".progress-fill").forEach((a,l)=>{setTimeout(()=>{a.classList.add("animate")},l*100)}))})},i);document.querySelectorAll(".fade-up, .fade-left, .fade-right, .fade-scale").forEach(e=>{t.observe(e)})},b=(i,t,e=2e3)=>{let s=0;const a=t/(e/16),l=i.dataset.suffix||"",n=()=>{s+=a,s<t?(i.textContent=Math.floor(s)+l,requestAnimationFrame(n)):i.textContent=t+l};n()},y=()=>{const i=document.querySelectorAll(".stat-number[data-target]"),t={threshold:.5},e=new IntersectionObserver(s=>{s.forEach(a=>{if(a.isIntersecting){const l=parseInt(a.target.dataset.target);b(a.target,l),e.unobserve(a.target)}})},t);i.forEach(s=>e.observe(s))},w=()=>{document.querySelectorAll(".btn-magnetic").forEach(i=>{i.addEventListener("mousemove",t=>{const e=i.getBoundingClientRect(),s=t.clientX-e.left-e.width/2,a=t.clientY-e.top-e.height/2;i.style.transform=`translate(${s*.3}px, ${a*.3}px)`}),i.addEventListener("mouseleave",()=>{i.style.transform="translate(0, 0)"})})},k=()=>{document.querySelectorAll(".project-card").forEach(i=>{i.addEventListener("mousemove",t=>{const e=i.getBoundingClientRect(),s=t.clientX-e.left,a=t.clientY-e.top,l=e.width/2,n=e.height/2,c=(a-n)/10,d=(l-s)/10;i.style.transform=`perspective(1000px) rotateX(${c}deg) rotateY(${d}deg) translateY(-15px)`}),i.addEventListener("mouseleave",()=>{i.style.transform="perspective(1000px) rotateX(0) rotateY(0) translateY(0)"})})},M=()=>{const i=document.querySelector("nav");i&&window.addEventListener("scroll",()=>{window.pageYOffset>50?(i.classList.add("glass-dark","py-4"),i.classList.remove("py-6")):(i.classList.remove("glass-dark","py-4"),i.classList.add("py-6"))})},j=()=>{document.querySelectorAll('a[href^="#"]').forEach(i=>{i.addEventListener("click",function(t){t.preventDefault();const e=document.querySelector(this.getAttribute("href"));e&&e.scrollIntoView({behavior:"smooth",block:"start"})})})},C=()=>{m.innerHTML=`
    <!-- Mouse Glow -->
  ...
  `};document.addEventListener("DOMContentLoaded",()=>{C(),requestAnimationFrame(()=>{u(),f(),h(),g(),y(),w(),k(),M(),j();const i=document.getElementById("mobile-menu-btn"),t=document.getElementById("mobile-menu");i&&t&&(i.addEventListener("click",()=>{t.classList.toggle("hidden")}),t.querySelectorAll("a").forEach(s=>{s.addEventListener("click",()=>{t.classList.add("hidden")})}));const e=document.createElement("style");e.textContent=`
